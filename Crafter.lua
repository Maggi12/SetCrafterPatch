...
local function addPatternToQueue(patternButton,i)
	...
	D(string.format("Crafter: pattern: [%d, %s], trait: [%d, %s]",
		requestTable["Pattern"][1], requestTable["Pattern"][2],
		requestTable["Trait"][1], requestTable["Trait"][2]
	))
...
local function findPatternName(patternIndex, station)
	...
	if station == CRAFTING_TYPE_CLOTHIER and patternIndex < 9 then
	...
	elseif station == CRAFTING_TYPE_BLACKSMITHING and patternIndex > 7 then
		weight = DolgubonSetCrafter.localizedStrings.armourTypes[1]
	else
		weight = ""
	end
	
	assert(station == CRAFTING_TYPE_CLOTHIER or
	       station == CRAFTING_TYPE_BLACKSMITHING or
...
local function AddForiegnSmithingRequest(pattern, isCP, level, styleIndex, traitIndex, useUniversalStyleItem, station, setIndex, quality, autocraft, reference, craftingObject)
	...
	if pattern and isCP ~= nil and level and styleIndex and traitIndex and station and setIndex and quality then
		...
		-- todo: maybe IsPatternWeapon(...), GetPatternWeightClass(...)
		if (queueTable.Weight[2] ~= "") or (station == CRAFTING_TYPE_WOODWORKING and pattern == 2) then
		...
		local msg = string.format(
			"pattern: %d [%s], weight: %s, style: %d [%s], trait: %d [%s]",
			queueTable.Pattern[1], queueTable.Pattern[2],
			queueTable.Weight[2],
			queueTable.Style[1], queueTable.Style[2],
			queueTable.Trait[1], queueTable.Trait[2]
		)
		--D(msg)
		--LLC_CraftSmithingItemByLevel(self, patternIndex, isCP , level, styleIndex, traitIndex, useUniversalStyleItem, stationOverride, setIndex, quality, autocraft)
